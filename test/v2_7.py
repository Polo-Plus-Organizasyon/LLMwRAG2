...
def get_conversation_chain():
    llm = ChatOllama(model= 'llama3',temperature=0.1, keep_alive=-1,repeat_last_n = 64,repeat_penalty = 1.1,top_k = 40,top_p = 0.9)
    
    template="""
    Verilen soruyu kurallara uygun olarak cevapla.
    # --------- #
    ### Kurallar ###
    1 - Cevaplarını Türkçe ver.
    2 - Türkçe karakterler ve Türkçe dil bilgisi kurallarına uygun kelimeler kullan.
    3 - İçeriği kaynak olarak kullan.
    4 - İçeriği kullanarak sorulara kendi cümlelerin ile cevap ver.
    5 - İçerikteki soruyla ilgisi olmayan cümleleri kullanma.
    6 - Sohbet geçmişi ile cevapların tutarlı olsun.
    7 - Verilen içerikte yeterli bilgi bulunmuyorsa, cevap vermekten kaçın ve kullanıcıya 'İçerikte yeterli bilgi yoktur.' yanıtını ver. Bu durumu belirlemek için, içeriğin şu kriterlere uygun olup olmadığını kontrol et: sorunun açıkça belirtilmiş olması, gerekli tüm detayların verilmiş olması ve cevabı oluşturmak için yeterli bağlamın bulunması. Eğer bu kriterler karşılanmıyorsa, yukarıdaki uyarıyı göster.
    8 - Sadece soruyu yanıtla.
    # --------- #
    İçerik: "{content}"
    \n
    # --------- #
...
class Grader:
    @staticmethod
    def text_corrector():
        llm = ChatOllama(model= 'llama3',temperature=0.5, keep_alive=-1,repeat_last_n = 64,repeat_penalty = 1.2,top_k = 50, top_p = 0.9, format='json', template="")
        ...
        prompt = PromptTemplate(
            ...
            "corrected_prompt": "mitoz bölünme açıkla"

            "original_prompt": "mtoz",
            "corrected_prompt": "mitoz"

            "original_prompt": "dinozre nedir",
            "corrected_prompt": "dinozor nedir"

            "original_prompt": "Tilskoop  nedir",
            "corrected_prompt": "Teleskop nedir"
...
def main():
    ...
    if st.session_state.processComplete:
        user_question = st.chat_input("Lütfen sorunuzu sorunuz?")
        if user_question:
...
async def user_input(user_question):
    ...
    truncated_history = st.session_state.chat_history[-6:]
    chat_history = "\n".join([f"Soru : {msg['question']}" if msg['role'] == "Öğrenci" else f"Cevap : {msg['content']}\n" for msg in truncated_history])
    print(chat_history)
    ...
    st.session_state.chat_history.append({"role": "Öğrenci",
                                        "avatar": '👨🏻',
    ...
    st.session_state.chat_history.append({"role": "Öğretmen",
                                        "avatar": '🤖',
