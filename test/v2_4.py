...
def get_conversation_chain():
    ...
    prompt = PromptTemplate(
        ...
        Sen çok bilgili bir asistansın ve aşağıdaki yönergelere mutlaka uyarak soruları yanıtlarsın:
        - Türkçe cevapla.
        - Kelimelerin doğru ve anlamlı yazıldığına ve doğru noktalama işaretlerinin kullanıldığına dikkat et.
        - Yalnızca verilen içeriğe dayanarak kendi cümlelerin ile yanıt ver.
        - İçerik bilgi içermiyorsa 'İçerikte yeterli bilgi bulunamadı.' yanıtı ver.
        - Yeni soruyu ve varsa sohbet geçmişini kullanarak cevap üret.
        - Soru hazırlaman gerekiyorsa, hazırladığın soru ve seçenekler alt alta olsun ve en son sorunun cevabını da yazmalısın.
        ...
        #------------------
        Yeni Soru:{question}<|eot_id|><|start_header_id|>user<|end_header_id|
        #------------------
...
async def user_input(user_question):
    ...
    response_stream = await loop.run_in_executor(None, st.session_state.conversation.stream, {'chat_history': chat_history, 'question': user_question, 'content': process_text(retriever_output=st.session_state.vectorstore.invoke(user_question))})
